...
def Editar(nome):
    JGC = tk.Tk()
    JGC.title(f"Janela de Edição de Curso")
    ...
    try:
        ...
        if conexao:
            ...
            if resultado:
                ...
                texto_vagas = tk.Label(JGC, text=f"Vagas Do Curso: {vag}", **config_text1)
                texto_vagas.place(x=20, y=50)

                texto_carga = tk.Label(JGC, text=f"Carga Horaria Do Curso: {carga_horaria}", **config_text1)
                ...
                texto_descricao = tk.Label(JGC, text=f"{desc}", **config_text1)
    ...
    bntDedeletar = tk.Button(JGC, text="Deletar", **config_botao, command=deletarDados)
    bntDedeletar.place(x=20, y=260)
    bntDeRegistro = tk.Button(JGC, text="Editar", **config_botao, command=salvarDados)
